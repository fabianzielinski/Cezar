...
function encryp(str, keyValue) {
	return str.split('').map((char) => {
		...
		if (x > 64 && x < 91) {
			console.log("A");
			console.log(((x - 65 + keyValue) % 26) + 65);
			return String.fromCharCode(((x - 65 + keyValue) % 26) + 65);
		} else if (x > 96 && x < 123) {
			console.log("a");
			console.log((((x - 97 + keyValue) % 26) + 97));
			let namber = x - 97 + parseInt(keyValue);
			console.log('x: ' + x);
			console.log('keyValue: ' + keyValue);
			console.log('namber: ' + namber);
			console.log('mod: ' + (namber % 26));
			return String.fromCharCode((((x - 97 + keyValue) % 26) + 97));
		} else {
			console.log('#');
